...
*   **Le risque d'imprécision technique et d'hallucination sur les finitions :** C'est le point le plus sensible pour la production.
    *   *Exemples initiaux :* Sur le soudage PVC (exigé à 245-250°C pour 25s), l'IA a arrondi à "240-270°C pendant quelques dizaines de secondes", ce qui est inacceptable pour un réglage machine. Sur la gamme GRAPHITE, elle a inventé une épaisseur de 42 mm et un Uw de 0,8 au lieu des réels 80 mm et 0,36. Sur la gamme TEXTURAL, elle a inventé des finitions "Marbre" et "Béton" au lieu de "Cuir" ou "Carbone".
    *   *Cas d'échec d'extraction des textures extérieures (TEXTURAL) :* Lors de tests sur les finitions extérieures de la gamme TEXTURAL®, le système a affirmé à tort qu'aucune finition bois n'était disponible à l'extérieur (concluant à un choix limité uniquement aux laquages RAL ou aspects alu brut). Le document source stipulait pourtant explicitement la faisabilité de deux textures bois extérieures : le **Chêne doré** et le **Chêne irlandais** dans la section *CATALOGUE DES TEXTURES EXTERIEURES* (page 3). Cet échec met en évidence un problème de cloisonnement du chunking où le titre de la section extérieure a été découplé sémantiquement des textures enfants.
*   **Confusion sur les Seuils Logiques, Dimensions et Redirections (Étude de Cas) :** Le système peine à appliquer des mathématiques simples ou des exceptions de pose si les limites ne sont pas rigidement indexées.
    *   *Exemple des perçages et renforts :* Il a affirmé qu'il fallait 2 perçages pour un profil de 1,50 m, ignorant la règle critique du "rajouter un perçage au milieu si Longueur > 1 m". Il a également estimé que les renforts de dormants étaient facultatifs "pour les petites fenêtres", omettant la condition vitale de la notice : "uniquement si fixé à la maçonnerie".
    *   *Cas de la Baie Coulissante Hors-Cote (INNOSLIDE vs LUMÉAL) :* Soumise à une demande d'architecte pour un coulissant de L 2,20 m x H 2,50 m par vantail, l'IA a mis du temps à rejeter la gamme PVC **INNOSLIDE** alors qu'elle dépasse sa limite de hauteur réglementaire (H max de 2,40 m, et L 2,10 m par vantail). De plus, l'IA a des difficultés à corréler cette contrainte pour rediriger automatiquement vers la solution aluminium **LUMÉAL** (acceptant jusqu'à H 2,70 m et 300 kg/vantail) en calculant le gain de clair de jour associé (+8% en applique et +12 à 14% en tunnel grâce à l'ouvrant caché).
*   **Glissements de lecture des garanties et historique temporel :**
    *   *Exemple des volets roulants :* Dans un tableau dense, le système s'est trompé de ligne, attribuant un Uw de 1,7 (valeur du 4 vantaux) au lieu de 1,5 (valeur du 2 vantaux). De plus, il a confondu l'historique des garanties des volets roulants, lissant les données et ignorant la baisse de garantie sur le tablier entre 2023 et 2024.
    *   *Cas des garanties différenciées sur Portes d'Entrée :* Lors de la configuration d'une porte d'entrée combinant un panneau plaxé, un soubassement mouluré et une crémone standard, le système a appliqué la garantie structurelle globale de 15 ans à l'ensemble du produit. En réalité, le référentiel technique (**Section 6**) détaille des garanties ultra-spécifiques et restrictives : **5 ans** pour le panneau plaxé (contre 7 ans en standard), **2 ans** pour le soubassement mouluré et **2 ans** pour la crémone/serrure.
*   **Angles morts documentaires et gestion des alertes critiques croisées (Multi-Hop) :**
    *   *Exemple d'incompatibilité de schémas :* Des schémas invisibles (ex: le fond de joint XL30202 uniquement présent en cartouche de dessin) et un échec d'indexation totale sur le catalogue de la gamme INNOSLIDE qu'elle juge "indisponible".
    *   *Cas d'alerte cumulée (Cintré + Teinte Sombre + Stores Intégrés) :* Soumis à une configuration de châssis cintré de couleur sombre avec stores intégrés dans le vitrage, l'IA n'a pas su lever immédiatement la double alerte critique :
