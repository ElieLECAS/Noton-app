---
type: Quincaillerie
title: Renvois d'angle et verrouilleurs Roto NX
description: Les références de commande, longueurs de branches et types de galets des renvois d'angle et verrouilleurs Roto NX pour menuiseries PVC.
tags: [roto, roto-nx, renvoi-angle, verrouilleur, prolongateur, galet, quincaillerie, reference]
famille: roto-nx
status: stable
sources:
  - resource: raw/roto-nx-catalogue-pvc-ctl-105-2023-06.pdf
    id: roto-nx-catalogue-ctl-105
    title: Roto NX, catalogue pour profils PVC, réf. CTL_105_FR_v5, juin 2023
    last_modified: 2023-06-30
  - resource: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf
    id: roto-nx-ksr-montage-imo-180
    title: Roto NX KSR, instructions de montage fenêtres et portes-fenêtres en PVC, réf. IMO_180_NX_FR_v2
    last_modified: 2022-11-30
source_pages:
  - resource: raw/roto-nx-catalogue-pvc-ctl-105-2023-06.pdf
    pages: 266-277, 318-333
  - resource: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf
    pages: 40-45, 78, 92-97
generated:
  by: process:multimodal-direct
  at: 2026-09-19T20:10:00Z
---

# Renvois d'angle et verrouilleurs Roto NX

Les renvois d'angle transmettent le mouvement de la crémone vers la traverse haute et les montants
verticaux de la menuiserie PVC [1 p. 266]. Les verrouilleurs médians assurent le serrage périphérique
de l'ouvrant sur le dormant, indispensable pour l'étanchéité à l'air A*4 et la résistance à l'effraction RC1 / RC2.

# Caractéristiques et cotes des renvois d'angle

Relevés sur le catalogue Roto NX CTL_105 (p. 266-277) et les instructions KSR (p. 40, 78).
Toutes les pièces reçoivent le traitement de surface anticorrosion **Roto Sil Level 6** (classe 5 DIN EN 1670).

### Renvois d'angle standard pour oscillo-battant et ouvrant à la française

| Désignation | Longueur branche horizontale (mm) | Longueur branche verticale (mm) | Type et nombre de galets | N° de matériel (article) |
| --- | --- | --- | --- | --- |
| Renvoi d'angle standard 1E | 135 | 135 | 1 E | 623190 |
| Renvoi d'angle standard 1P | 135 | 135 | 1 P | 623191 |
| Renvoi d'angle sécurité 1V | 135 | 135 | 1 V | 623192 |
| Renvoi d'angle sans galet | 135 | 135 | sans | 623189 |
| Renvoi d'angle à branche courte | 98 | 135 | sans | 742198 |
| Renvoi d'angle à branche longue 1E | 170 | 135 | 1 E | 625615 |
| Renvoi d'angle à branche longue 1P | 170 | 135 | 1 P | 625627 |
| Renvoi d'angle à branche longue 1V | 170 | 135 | 1 V | 625628 |

(schéma: raw/roto-nx-catalogue-pvc-ctl-105-2023-06.pdf, p. 268-272)

### Renvois d'angle spécifiques pour sécurité renforcée et aération TiltSafe

| Application | Spécificité | Type de galet | N° de matériel |
| --- | --- | --- | --- |
| Renvoi d'angle TiltSafe RC2 | Galet de sécurité renforcé pour position entrouverte | 1 V | 794645 |
| Renvoi d'angle d'aération nocturne | Course de décompression 10 mm | 1 E | 623195 |
| Renvoi d'angle d'aération nocturne | Course de décompression 10 mm | 1 V | 623196 |
| Renvoi d'angle avec loqueteau | Intègre le blocage de ventelle | 1 E | 795325 |

(schéma: raw/roto-nx-catalogue-pvc-ctl-105-2023-06.pdf, p. 274-276)

# Caractéristiques et cotes des verrouilleurs médians

Relevés sur le catalogue Roto NX (p. 318-333) et les instructions KSR (p. 78).
Les verrouilleurs s'installent côté paumelles (horizontalement en traverse haute ou verticalement en montant arrière)
pour reprendre les déformations de profil sous pression de vent.

### Verrouilleurs médians horizontaux (traverse haute côté compas)

| Plage largeur fond de feuillure LFF (mm) | Longueur de pièce (mm) | Type et nombre de galets | N° de matériel |
| --- | --- | --- | --- |
| 801 – 1 200 | 250 | 1 E | 623605 |
| 801 – 1 200 | 250 | 1 V | 623610 |
| 1 201 – 1 600 | 500 | 1 E | 623606 |
| 1 201 – 1 600 | 500 | 1 V | 623611 |

(schéma: raw/roto-nx-catalogue-pvc-ctl-105-2023-06.pdf, p. 320-324)

### Verrouilleurs médians verticaux (montant arrière côté paumelle)

| Plage hauteur fond de feuillure HFF (mm) | Longueur de pièce (mm) | Nombre de galets | Galet E | Galet V |
| --- | --- | --- | --- | --- |
| 801 – 1 200 | 425 | 1 | 623604 | 623612 |
| 1 201 – 1 600 | 675 | 1 | 623645 | 623647 |
| 1 601 – 2 000 | 875 | 2 | 375797 | 623648 |
| 2 001 – 2 400 | 1 075 | 2 | 375799 | 623649 |

(schéma: raw/roto-nx-catalogue-pvc-ctl-105-2023-06.pdf, p. 326-330)

### Allonges et prolongateurs de verrouilleurs

| Désignation | Longueur (mm) | Nombre de galets | N° de matériel |
| --- | --- | --- | --- |
| Allonge de tringle courte | 200 | sans galet | 767639 |
| Allonge de tringle moyenne | 400 | 1 E | 280346 |
| Allonge de tringle moyenne | 400 | 1 V | 280347 |
| Allonge de tringle longue | 600 | 1 E | 255282 |
| Allonge de tringle longue | 600 | 1 V | 255283 |

(schéma: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf, p. 78)

# Champs d'application et règles de pose

- **Seuil d'implantation du verrouilleur médian horizontal** : obligatoire dès que la largeur en fond de feuillure **LFF dépasse 1 200 mm** (ou 800 mm en classe RC2 / retard à l'effraction) [2 p. 40, 92].
- **Seuil d'implantation du verrouilleur vertical arrière** : obligatoire dès que la hauteur en fond de feuillure **HFF dépasse 1 200 mm** en sécurité de base et dès 800 mm en RC2 [2 p. 40, 94].
- **Recoupe** : les tringles de liaison et verrouilleurs possèdent des zones de recoupe crantées permettant l'ajustement au millimètre sur banc de ferrage atelier.
- **Raccordement Clip&Fit** : jonction brevetée par emboîtement élastique sans vis de raccordement intermédiaire.

# Ce que la source ne donne pas

- La compatibilité avec les profilés à rainure de ferrage de 9 mm (le catalogue PVC CTL_105 cible exclusivement l'axe de 13 mm).

# Citations

[1] Roto NX, catalogue pour profils PVC, réf. CTL_105_FR_v5, juin 2023 — `raw/roto-nx-catalogue-pvc-ctl-105-2023-06.pdf`, p. 266 à 277 et 318 à 333
[2] Roto NX KSR, instructions de montage fenêtres et portes-fenêtres en PVC, réf. IMO_180_NX_FR_v2, novembre 2022 — `raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf`, p. 40 à 45, 78 et 92 à 97

# Voir aussi

- [Crémones Roto NX](/quincaillerie/roto-nx-cremones.md)
- [Compas et paliers Roto NX](/quincaillerie/roto-nx-compas-et-paliers.md)
- [Pièces de fermeture et gâches Roto NX](/quincaillerie/roto-nx-pieces-fermeture-gaches.md)
- [Champs d'application Roto NX](/quincaillerie/roto-nx-champs-application.md)
- [Roto NX](/quincaillerie/roto-nx.md)
- [Catalogue Roto NX pour profils PVC](/sources/roto-nx-catalogue-pvc.md)
