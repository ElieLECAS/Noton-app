---
type: Quincaillerie
title: Configurations Roto NX KSR — ouvrant à la française
description: Les champs d'application, la quincaillerie et le positionnement des gâches de l'ouvrant à la française Roto NX KSR, à un ou deux vantaux, avec ses compas, têtières, verrouilleurs médians et gabarits de montage.
tags: [roto, roto-nx, ksr, ouvrant-francaise, of, quincaillerie, champs-application]
famille: roto-nx
status: stable
sources:
  - resource: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf
    id: roto-nx-ksr-montage-imo-180
    title: Roto NX KSR, instructions de montage fenêtres et portes-fenêtres en PVC, réf. IMO_180_NX_FR_v2
    last_modified: 2022-11-30
source_pages:
  - resource: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf
    pages: 36-39, 46, 48, 50, 98, 104
generated:
  by: process:claude-code
  at: 2026-09-19T23:55:00Z
---

# Ouvrant à la française à un vantail

Champs d'application, relevés sur la planche de configuration [1 p. 36] :

| Grandeur | Plage |
| --- | --- |
| Largeur en feuillure du vantail | 290 à 1600 mm |
| Hauteur en feuillure du vantail | 300 à 2800 mm |
| Poids du vantail | 130 ou 150 kg maximum |

La crémone se choisit dans [Crémones Roto NX](/quincaillerie/roto-nx-cremones.md), famille
fouillot 8 mm hauteur de poignée fixe. Les paliers, pivots et compas OF communs sont sur
[Composants communs Roto NX KSR](/quincaillerie/roto-nx-composants-communs.md).

Quincaillerie propre à cette configuration :

| Pièce | Détail | Référence |
| --- | --- | --- |
| Verrouilleur médian horizontal, largeur en feuillure 801-1200 mm | longueur 400 mm | 255280 |
| Verrouilleur médian horizontal, largeur en feuillure 1201-1400 mm | longueur 600 mm | 255281 |
| Verrouilleur médian horizontal, largeur en feuillure 1401-1600 mm | longueur 600 KU | 255282 |
| Prolongateur de 110 mm, galet E | — | 794225 |
| Renvoi d'angle | galet V | 260272 |

(schéma: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf, p. 36)

# Ouvrant à la française à deux vantaux

Champs d'application [1 p. 38] :

| Grandeur | Plage |
| --- | --- |
| Largeur en feuillure du vantail | 290 à 1600 mm |
| Hauteur en feuillure du vantail | 375 à 2800 mm |
| Poids du vantail | 130 ou 150 kg maximum |

Options propres à cette configuration, en plus des composants communs et de la crémone :

| Pièce | Détail | Référence |
| --- | --- | --- |
| Releveur | option | 795925 |
| Gâche releveur sur battement | option | 623126 |
| Verrou pour semi-fixe | longueur 200 | 633419 |
| Verrou pour semi-fixe | longueur 390 | 618666 |
| Cale ouvrant pour loqueteau | — | 794997 |
| Prolongateur sortie de tringle de 14 mm | — | 349187 |
| Renvoi d'angle | galet V | 260272 |
| Renvoi d'angle OB | galet V, avec renvoi d'angle intégré | 260288 |
| Mini renvoi d'angle | galet P | 260282 |
| Gâche sur battement | — | 623125 |
| Renvoi de fouillot | option pour poignée centrée, voir fichier gamme | — |

(schéma: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf, p. 38)

**Aucun renvoi d'angle n'est possible sur les hauteurs de feuillure couvertes par le mini renvoi
d'angle** (601-700 mm, ou 300-400 mm en version courte HP fixe) [1 p. 38].

# Ouvrant à la française, crémone à sortie de tringle

Trois variantes de la même quincaillerie de base, qui se distinguent par leurs champs
d'application [1 p. 46, 48 et 50] :

| Configuration | Largeur en feuillure (mm) | Hauteur en feuillure (mm) | Poids du vantail (kg) |
| --- | --- | --- | --- |
| Un et deux vantaux | 290 – 1600 | 375 – 2400 | 130 ou 150 max. |
| Deux vantaux larges, poignée centrée | 800 – 1600 | 375 – 2400 | 130 ou 150 max. |
| Deux vantaux, crémone à sortie de tringle et crémone de semi-fixe | 290 – 1600 | 601 – 2400 | 130 ou 150 max. |

La crémone à sortie de tringle, son prolongateur et la crémone de semi-fixe associée sont sur
[Crémones Roto NX](/quincaillerie/roto-nx-cremones.md). L'équerre de compas OF, le compas OF, les
paliers et le pivot d'angle communs sont sur
[Composants communs Roto NX KSR](/quincaillerie/roto-nx-composants-communs.md).

Options propres à ces trois configurations :

| Pièce | Configuration | Référence |
| --- | --- | --- |
| Renvoi d'angle | un et deux vantaux, larges | 383480 |
| Renvoi d'angle avec galet V | un et deux vantaux, larges | 553749 |
| Verrouilleur médian horizontal, 800-1200 mm | larges, sortie de tringle | 255280 |
| Verrouilleur médian horizontal, 1201-1600 mm | larges, sortie de tringle | 255281 |
| Verrou pour semi-fixe 200 | toutes | 633419 |
| Verrou pour semi-fixe 390 | toutes | 618666 |
| Gâche sur battement | toutes | 623125 |
| Renvoi de fouillot | option pour poignée centrée, voir fichier gamme | - |

(schéma: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf, p. 46, 48 et 50)

# Positionnement des gâches — crémone à sortie de tringle

Cotes de positionnement des gâches (dormant, deux vantaux sans meneau fixe), crémone à sorties de
tringle, fouillot 15, 8 et -6 mm, hauteur de poignée fixe [1 p. 98]. **HP** est la cote de la
crémone, **G1** se relève depuis le haut et **AFM** depuis le bas, chacune selon la hauteur en
feuillure HFF :

| Crémone OF F8 — HFF (mm) | HP (mm) | G1 (mm) | AFM (mm) |
| --- | --- | --- | --- |
| 375 – 600 | 120 | - | - |
| 601 – 800 | 220 | 391 | - |
| 801 – 1000 | 413 | 540 | 240 |
| 1001 – 1200 | 513 | 640 | 313 |
| 1201 – 1800 | 563 | 690 | 363 |
| 1801 – 2400 | 763 | 392 | 563 |
| 1801 – 2400 | 1000 | 622 | 800 |

| Crémone OF F-6 — HFF (mm) | HP (mm) | G1 (mm) | AFM (mm) |
| --- | --- | --- | --- |
| 375 – 600 | 120 | - | - |
| 601 – 800 | 220 | 391 | 80 |
| 450 – 600 | 413 | 540 | 213 |
| 1001 – 1200 | 513 | 640 | 313 |
| 1201 – 1800 | 563 | 690 | 363 |
| 1801 – 2400 | 763 | 392 | 563 |
| 1801 – 2400 | 1000 | 622 | 800 |

| Crémone OF F15 — HFF (mm) | HP (mm) | G1 (mm) | AFM (mm) |
| --- | --- | --- | --- |
| 375 – 559 | 120 | - | - |
| 450 – 600 | 170 | - | - |
| 601 – 800 | 263 | - | 90 |
| 801 – 1000 | 413 | 540 | 240 |
| 1001 – 1200 | 513 | 640 | 313 |
| 1201 – 1800 | 563 | 690 | 363 |
| 1801 – 2400 | 1000 | 622 | 800 |

(schéma: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf, p. 98)

**Allonge OF**, pour les grandes hauteurs :

| Longueur (mm) | Lg (mm) | G2 (mm) |
| --- | --- | --- |
| 1200 – 1400 | 675 | 343 |
| 1801 – 2000 | 875 | 543 |
| 2001 – 2200 | 1075 | 743 |

**Verrouilleur médian horizontal** :

| Largeur FF (mm) | MH1 (mm) | Référence |
| --- | --- | --- |
| 800 – 1200 | 520 | 400 E |
| 1201 – 1600 | 720 | 600 E |

(schéma: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf, p. 98)

# Gabarit de montage OF, deux vantaux — utilisation

| Gabarit | Utilisation |
| --- | --- |
| OB H 03 | horizontal télescopique haut et bas |
| OF V 06 | gâches crémones HP 220, 763, 1000 mm, fouillot 8 et -6 (butée jaune) ; allonges AL 675, 875, 1075 (butée bleue) |
| OF V 07 | gâches crémones HP 413, 513, 563 mm, fouillot 15, 8 et -6 (butée jaune) ; allonges AL 675, 875, 1075 (butée bleue) |

(schéma: raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf, p. 104)

# Ce que la source ne donne pas

Les cotes de montage du verrouilleur médian selon la hauteur en feuillure ne sont pas détaillées
au-delà des trois plages de largeur ci-dessus : la source ne croise pas largeur et hauteur pour
cette pièce sur ces deux configurations. Sur la table de la crémone à sortie de tringle (p. 98),
la plage 1801-2400 mm porte deux jeux de cotes G1/AFM sans que le document précise le critère de
choix entre eux — probablement une variante haute/basse de la même plage, à vérifier.

# Citations

[1] Roto NX KSR, instructions de montage fenêtres et portes-fenêtres en PVC, réf.
IMO_180_NX_FR_v2, novembre 2022 — `raw/roto-nx-ksr-montage-pvc-imo-180-2022-11.pdf`, p. 36 à 39,
46, 48, 50, 98 et 104

# Voir aussi

- [Roto NX](/quincaillerie/roto-nx.md)
- [Crémones Roto NX](/quincaillerie/roto-nx-cremones.md)
- [Composants communs Roto NX KSR](/quincaillerie/roto-nx-composants-communs.md)
- [Champs d'application Roto NX](/quincaillerie/roto-nx-champs-application.md)
- [Instructions de montage Roto NX KSR](/sources/roto-nx-ksr-montage.md)
- [ROTO](/fournisseurs/roto.md)
