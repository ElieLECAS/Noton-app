---
type: Document source
title: Mise en œuvre 9708, profine France
description: Fiche d'une page pour le redressement d'un montant de porte cintré au moyen du profilé acier précontraint 9708.
tags: [profine, 9708, montant-cintre]
status: stable
sources:
  - resource: raw/profine-mise-en-oeuvre-9708-montants-cintres-2017-07.pdf
    id: profine-9708-montants-cintres
    title: Mise en œuvre 9708, profine France, juillet 2017
    last_modified: 2017-07-06
generated:
  by: process:claude-code
  at: 2026-09-19T22:30:00Z
---

# Identité

| Élément | Valeur |
| --- | --- |
| Éditeur | profine France |
| Date | 6 juillet 2017 |
| Pages | 1 |
| Nature | fiche isolée, atelier et chantier |

# Registre de couverture

| Pages PDF | Contenu | État | Page du wiki |
| --- | --- | --- | --- |
| 1 | préconisation complète : condition d'exclusion, six étapes, calage selon le sens du cintre, cotes | transcrit | [Redressement d'un montant de porte cintré par le profilé 9708](/procedures/redressement-montant-porte-cintre-9708.md) |

# Citations

[1] Mise en œuvre 9708, profine France, juillet 2017 —
`raw/profine-mise-en-oeuvre-9708-montants-cintres-2017-07.pdf`

# Voir aussi

- [Redressement d'un montant de porte cintré par le profilé 9708](/procedures/redressement-montant-porte-cintre-9708.md)
- [profine](/fournisseurs/profine.md)
