...
PAGE_ENRICHMENT_PROMPT_TEMPLATE = """Tu es un ingénieur en intelligence artificielle spécialisé dans le traitement de documents techniques de menuiserie (fenêtres, coulissants, tolérances, garanties, notices de pose).
Analyse le texte et les tableaux de la page suivante d'un document technique et extrais des informations structurées de haute qualité.

Tu dois retourner UNIQUEMENT un JSON valide, sans balises markdown (comme ```json), sans texte avant ni après.

Types d'entités techniques à extraire (taxonomie KAG) :
{entity_types}

### Consignes de génération :
1. **summary** : Rédige un résumé technique, factuel et dense (1 à 2 phrases max) de la page. Ne fais pas de phrases d'introduction creuses ("Cette page contient..."). Sois direct et technique.
2. **qas** : Génère exactement 3 couples Question/Réponse. Les questions doivent porter sur des détails techniques cruciaux (limites dimensionnelles, tolérances thermiques, règles de pose, accessoires). Les réponses doivent être courtes, précises, et directement présentes dans le texte.
3. **entities** : Extrais jusqu'à 10 entités de cette page. Pour chaque entité, indique son "name" (nom exact dans le texte, ex: "Luméal", "soudure grain d'ange"), son "type" (choisi strictement parmi la liste ci-dessus) et son "importance" (de 0.0 à 1.0).

### Format JSON attendu (doit être un dictionnaire valide) :
{{
  "summary": "Résumé ultra-dense...",
  "qas": [
    {{
      "question": "Question technique 1 ?",
      "answer": "Réponse technique 1..."
    }},
    {{
      "question": "Question technique 2 ?",
      "answer": "Réponse technique 2..."
    }},
    {{
      "question": "Question technique 3 ?",
      "answer": "Réponse technique 3..."
    }}
  ],
  "entities": [
    {{"name": "Nom de l'entité", "type": "type_entite", "importance": 0.9}}
  ]
}}
...
def _parse_page_enrichment_response(response_text: str) -> Optional[Dict]:
    """
    Parse la réponse LLM unifiée pour l'enrichissement par page (summary + qas + entities).
    """
    if not response_text:
        return None
    ...
    try:
        ...
        if isinstance(entities, list):
            for e in entities:
                if isinstance(e, dict):
                    ...
                    if not name or len(name) < 2:
                        continue
                    if raw_type not in SUPPORTED_ENTITY_TYPE_IDS:
                        continue
                    if not isinstance(importance, (int, float)):
                        importance = 1.0
                    importance = max(0.0, min(1.0, float(importance)))
        ...
        return {
            "summary": summary,
            "qas": valid_qas[:5],
            "entities": valid_entities[:10]
        }
    except json.JSONDecodeError as e:
        logger.warning("Erreur parsing JSON enrichissement unifié de page: %s - Réponse: %s", e, text[:200])
        return None


async def generate_page_enrichment_and_entities(
    page_content: str,
    context_hint: Optional[str] = None
) -> Optional[Dict]:
    """
    Génère le résumé, les 3 Q&As et extrait les entités pour une page en un seul appel LLM.
    """
    if not page_content or len(page_content.strip()) < 30:
        return None

    page_content_truncated = page_content[:6000]
    context_section = f"Contexte additionnel : {context_hint}" if context_hint else ""
    
    prompt = PAGE_ENRICHMENT_PROMPT_TEMPLATE.format(
        entity_types=ENTITY_TYPES_PROMPT_BLOCK,
        context_section=context_section,
        page_content=page_content_truncated
    )
    ...
    try:
        if provider == "openai":
            ...
            logger.error("Provider KAG inconnu pour enrichissement page: %s", provider)
            return None

        return _parse_page_enrichment_response(raw)

    except Exception as e:
        logger.error("Erreur génération enrichissement page unifiée: %s", e, exc_info=True)
        return None


def generate_page_enrichment_and_entities_sync(
    page_content: str,
    context_hint: Optional[str] = None
) -> Optional[Dict]:
    """
    Version synchrone de generate_page_enrichment_and_entities.
    """
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as pool:
                future = pool.submit(
                    asyncio.run,
                    generate_page_enrichment_and_entities(page_content, context_hint),
                )
                return future.result(timeout=120)
        else:
            return loop.run_until_complete(generate_page_enrichment_and_entities(page_content, context_hint))
    except RuntimeError:
        return asyncio.run(generate_page_enrichment_and_entities(page_content, context_hint))
    except Exception as e:
        logger.warning("Génération enrichissement page unifiée échouée: %s", e)
        return None
